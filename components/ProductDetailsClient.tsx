...
import { Check, Phone, Package, Shield, ArrowLeft, Share2 } from "lucide-react";
...
export default function ProductDetailsClient({ initialProduct }: { initialProduct: Product }) {
  ...
  const productUrl = `https://shivamwatersolution.in/products/${product.id}`;
  const defaultWaText = `Hello Shivam Water Solution (Dilipbhai)! 👋\n\nI am interested in inquiring about the *${product.name}* ${product.category === 'commercial' ? 'Commercial RO Plant' : 'RO Water Purifier'}.\n\n📦 *Specs:* ${product.capacity || 'Standard Capacity'} | ${product.warranty || '1 Year Warranty'}\n🚚 *Services:* Free Delivery & Free Installation in Morbi/Rajkot\n\n🔗 *Product Details & Photo:* ${productUrl}`;
  const waUrl = `https://wa.me/919173096727?text=${encodeURIComponent(product.wa ? `${product.wa}\n\n🔗 Product Link: ${productUrl}` : defaultWaText)}`;
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          ...
          text: `Check out ${product.name} RO Water Purifier from Shivam Water Solution Morbi!`,
          url: productUrl,
        });
        return;
      } catch {
      ...
      <div className="container">
        ...
        <div className="product-detail-grid">
          ...
          <div className="product-gallery-container">
            <div className="product-image-section glass-card" style={{ position: "relative" }}>
              <button
                type="button"
                onClick={handleShare}
                className={`share-floating-btn ${copied ? "copied" : ""}`}
                aria-label="Share product"
                title={copied ? "Link Copied! ✓" : "Share product link"}
              >
                <Share2 size={18} />
                <span className="share-tooltip">{copied ? "Copied! ✓" : "Share"}</span>
              </button>
          ...
          <div className="product-info-section">
            ...
            <div className="action-buttons-detail">
              ...
              <button
                type="button"
                onClick={handleShare}
                className={`share-action-bar mt-3 ${copied ? "copied" : ""}`}
                title="Share product page link"
              >
                <Share2 size={16} />
                <span>{copied ? "Product Link Copied to Clipboard! ✓" : "Share This Product Link"}</span>
              </button>
            </div>
          </div>
        </div>
